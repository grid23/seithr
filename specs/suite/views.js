...
import { expression, Model, ZParser, View } from "seithr"
...
describe("View.expression", () => {
    ...
    it("View.expression`div + ${View.expressWith(...args)}`", done => {
        ...
        const e = View.x`div + ${W.xw("foo", "bar")}`
        ZParser.parse(e)
    })
    ...
    it("View.expression`div{${model.m.value[0]}}`", () => {
        ...
        expect(ZParser.parse(e).fragment.childNodes[0].textContent === "foo")
    })
...
describe("ZParser", () => {
    it("ZParser.parse(div > span)", () => {
        const p = ZParser.parse(expression`div > span`)
    ...
    it("ZParser.parse(div > span > b)", () => {
        const p = ZParser.parse(expression`div > span > b`)
    ...
    it("ZParser.parse(div + div)", () => {
        const p = ZParser.parse(expression`div + div`)
    ...
    it("ZParser.parse(div + div > span)", () => {
        const p = ZParser.parse(expression`div + div > span`)
    ...
    it("ZParser.parse(div > span + span)", () => {
        const p = ZParser.parse(expression`div > span + span`)
    ...
    it("ZParser.parse(div@foo > span@foo)", () => {
        const p = ZParser.parse(expression`div@foo >span@foo`)
        expect(p.fragment.childNodes.length == 1).to.be.true
        expect(p.fragment.childNodes[0].childNodes.length == 1).to.be.true
        expect(p.refs.root.length == 1).to.be.true
    ...
    it("ZParser.parse((span#foo@foo) + (span#bar@bar) + span#fu@fu))", () => {
        const p = ZParser.parse(expression`(span#foo@foo) + (span#bar@bar) + span#fu@fu`)

        expect(p.refs.root.length === 3).to.be.true
        expect(p.refs.root[0].id === "foo").to.be.true
        expect(p.refs.root[1].id === "bar").to.be.true
        expect(p.refs.root[2].id === "fu").to.be.true
    })

    it("ZParser.parse(div@foo + div@bar > span@foo)", () => {
        const p = ZParser.parse(expression`div@foo + div@bar > span@foo`)
        expect(p.fragment.childNodes.length == 2).to.be.true
    ...
    it("ZParser.parse(div[foo])", () => {
        const p = ZParser.parse(expression`div[foo] > span[foo=bar]`)
    ...
    it("ZParser.parse(div[foo=${model.io.value}])", () => {
        const m = new Model
        m.io = { foo: "bar" }
        const p = ZParser.parse(expression`div[foo=${m.io.foo}]`)
    ...
    it("ZParser.parse(div[foo=${model.m.value}])", () => {
        ...
        const p = ZParser.parse(expression`div[foo=${m.m.foo}]`)
    ...
    it("ZParser.parse(div[foo=foo-${model.m.value}-bar-${model.m.value}])", () => {
        ...
        const p = ZParser.parse(expression`div[foo=foo-${m.m.foo}-bar-${m.m.bar}]`)
        expect(p.fragment.childNodes[0].getAttribute("foo") === "foo-abc-bar-def").to.be.true
    })

    it("ZParser.parse(div#foo)", () => {
        const p = ZParser.parse(expression`div#foo`)
    ...
    it("ZParser.parse(div#${model.io.value})", () => {
        const m = new Model
        m.io = { foo: "bar" }
        const p = ZParser.parse(expression`div#${m.io.foo}`)
    ...
    it("ZParser.parse(div#foo-${model.m.value}-bar-${model.m.value})", () => {
        ...
        const p = ZParser.parse(expression`div#foo-${m.m.foo}-bar-${m.m.bar}`)
    ...
    it("ZParser.parse(div.foo.bar)", () => {
        const p = ZParser.parse(expression`div.foo.bar`)
    ...
    it("ZParser.parse(div.${model.io.value})", () => {
        ...
        const p = ZParser.parse(expression`div.${m.m.foo}`)
    ...
    it("ZParser.parse(div.${model.m.value}.${model.m.value})", () => {
        ...
        const p = ZParser.parse(expression`div.${m.m.foo}.${m.m.bar}`)
    ...
    it("ZParser.parse(div.${model.m.value}.${model.m.value}.pqr)", () => {
        ...
        const p = ZParser.parse(expression`div.${m.m.foo}.${m.m.bar}.pqr`)
    ...
    it("ZParser.parse(div{hello world!})", () => {
        const p = ZParser.parse(expression`div{hello world!}`)
    ...
    it("ZParser.parse(div{hello ${m.m.value}})", () => {
        const m = new Model
        m.io = { world: "world" }
        const p = ZParser.parse(expression`div{hello ${m.m.world}!}`)
    ...
    it("WARN: ZParser.parse(div{hello }{{${m.m.value}})", done => {
        ...
        ZParser.parse(expression`div{hello }{{${m.m.world}!}}`)
    })
    ...
    it("ZParser.parse(ul > (li > span > (p@p+p@p)) + (li > span > (p@p + p@p)))", () => {
        ...
        const p = ZParser.parse(expression`ul@ul > (li > span > (p@p+p@p)) + (li > span > (p@p + p@p))`)
    ...
    it("ZParser.parse(ul > ${A} + ${B.xw(args)})", () => {
        const m = new Model
        ...
        const p = ZParser.parse(expression`ul@ul > ${A} + ${B.xw({ props: {world: m.io.world} })}`)
    ...
    it("scope of evaluation (=> evaluation shouldn't overflow to global scope)", () => {
        ...
        const p = ZParser.parse(expression`div[id=${m.m.c}]`)
        const q = ZParser.parse(expression`div[foo=${m.m.d}]`)
        const r = ZParser.parse(expression`div[foo=${m.m.e}]`)
        const s = ZParser.parse(expression`div[foo=${m.m.a}]`)
        const t = ZParser.parse(expression`div[foo=${m.m.b}]`)
