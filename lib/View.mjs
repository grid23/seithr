...
export default class View extends Node {
    constructor(...args){
        ...
        const template = typeOf(dict.template) == "string" || dict.template instanceof ZExpression
                          ? dict.template
                       : typeOf(this.template) == "string" || this.template instanceof ZExpression
                          ? this.template
                       : typeOf(this._template) == "string" || this._template instanceof ZExpression
                          ? this._template
                       : ""
        ...
        this.appendChild(this.ztemplate)
    }

    get expression(){ return store.get(this).get(sexpression) }
    get fragment(){ return this.ztemplate.tree }
    get model(){ return store.get(this).get(smodel) }
    get references(){ return this.refs }
    get refs(){ return this.ztemplate.references }
    get root(){
        const root = this.queryAll("root")
        return root.length == 1 ? root[0] : root
    }

    get vars(){ return this.ztemplate.vars }
    get ztemplate(){ return store.get(this).get(stemplate) }
