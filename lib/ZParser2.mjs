...
import View, { sargs, sconstructor, xmap } from "./View2.mjs"
...
operators.set(".", {
  ...
  , handler(captured){
        ...
        const update = {
            updaters
          , handler: function(node, handler){
            // at the last update, what were the classnames, position, and length
                const was = []

                return function(){
                    const onnode = node.className.trim().split(" ")
                    const position = !onnode.length ? 0 // if node.className is empty, assume index 0
                                   : !was.length ? onnode.length // if old classes is empty, assume next in line
                                   : onnode.indexOf(was[0]) // else index of the first old classname
                    const change = handler().split(" ")
                    const to = [...onnode]
                    to.splice(position, was.length, ...change)

                    node.setAttribute("class", escapeHTML( to.join(" ").trim() ))
                    was.splice(0, was.length, ...change)
                }
            }(this.buffer, handler)
        }
        ...
        const update = {
          ...
          , handler: safe
              ? function(node, handler){
                    return function(){ node.nodeValue = escapeHTML(handler()) }
                }(node, handler)
              : function(node, handler){
                    let was = ""
                    return function(){
...
operators.set("|", {
  ...
  , handler(capture){
        ...
        const symbol = Symbol.for(capture)
        const View = xmap.get(symbol).get(sconstructor)
        const args = xmap.get(symbol).get(sargs) || []

        this.buffer = new View(...args).fragment
    }
  , capture(){
        do {
            const { done, value } = this.lookAhead()
